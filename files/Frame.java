...
class Frame {
  ...
  static int cmp(int c0,int c1) {
    if(c0<0) return c1<0 || c1==' ' || c1=='　' ? 0 : 1;
    if(c1<0) return c0==' ' || c0=='　' ? 0 : 1;
    return c0==c1 ? 0 : 1;
  }
  ...
  static void display(List sl) {
    ...
    for(int i=0,n=sl.size();i<=n;i++) {
      ...
      for(int j=0;j<=m;j++) {
        int k=cmp(get(s0,j-1),get(s,j-1))+cmp(get(s0,j),get(s,j))*2+
              cmp(get(s0,j-1),get(s0,j))*4+cmp(get(s,j-1),get(s,j))*8;
